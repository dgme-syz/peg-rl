...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_embed = self.config.embedding.enable

        if self.use_comet:
            m_list = list(self.config.comet_model.get("model_list", []))
            if (n := self.config.comet_model.get("n", 0)) < 0 or len(m_list) <= 0:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if self.use_comet:
                ...
                for i in range(len(self.comet_names)):
            ...
            if self.use_embed:
                ...
                print(f"Before pad, test output gen batch length: {len(test_output_gen_batch)}")
                test_output_gen_batch, embed_pad_size = pad_dataproto_to_divisor(test_output_gen_batch, embed_size_divisor)
                embed_reward = self.embed_wg.compute_valid_embed(test_output_gen_batch)
                embed_reward = unpad_dataproto(embed_reward, pad_size=embed_pad_size)
                print(f"Unpad, Embed model, length: {len(embed_reward)}")
                embed_reward_tensor = embed_reward.batch[f'embed_valid']
                embed_scores = embed_reward_tensor.sum(-1).cpu().tolist()
                reward_extra_infos_dict[f'embed_valid'].extend(embed_scores)

                test_output_gen_batch = unpad_dataproto(test_output_gen_batch, pad_size=embed_pad_size)
    ...
    def init_workers(self):
        ...
        if self.use_embed:
            ...
            embed_cls = RayClassWithInitArgs(self.role_worker_mapping[Role.EMBEDDING], config=self.config.embedding)
            self.resource_pool_to_cls[resource_pool][str(Role.EMBEDDING)] = embed_cls

        if self.use_comet:
            self.comet_names = []
            from omegaconf import open_dict
            _n = self.config.comet_model.get("n", 1)
            n = len(list(self.config.comet_model.model_list))
            for i in range(n):
                name = getattr(Role, f"COMET_{i}")
                resource_pool = self.resource_pool_manager.get_resource_pool(name)
                comet_cfg = OmegaConf.create(OmegaConf.to_yaml(self.config.comet_model))
                with open_dict(comet_cfg):
                    (
                        comet_cfg.model_name,
                        comet_cfg.ckpt_path
                    ) = list(self.config.comet_model.model_list)[i]
                self.comet_names.append(comet_cfg.model_name)
                if i + 1 <= _n:
                    print(f"\033[92muse {comet_cfg.model_name} as our reward+valid model\033[0m")
                else:
                    print(f"\033[91muse {comet_cfg.model_name} as our valid model\033[0m")
                comet_cls = RayClassWithInitArgs(self.role_worker_mapping[name], config=comet_cfg)
        ...
        if self.use_embed:
            self.embed_wg = all_wg[str(Role.EMBEDDING)]
            self.embed_wg.init_model()

        if self.use_comet:
            self.comet_wg_list = []
            n = len(self.comet_names)
            for i in range(n):
                na = getattr(Role, f"COMET_{i}")
                self.comet_wg_list.append(all_wg[str(na)])
                self.comet_wg_list[i].init_model()
    ...
    def _start_profiling(self, do_profile: bool) -> None:
        ...
        if do_profile:
            ...
            if self.use_embed:
                self.embed_wg.start_profile(profile_step=self.global_steps)
            if self.use_comet:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        # compute comet score
                        if self.use_comet:
                            m_list = self.config.comet_model.get("model_list", [])
                            if (n := self.config.comet_model.get("n", 0)) < 0 or len(m_list) <= 0:
                                raise ValueError(f"You wants to use comet model as rm, but use n = {n}, and model_list = {m_list}")
                            for i in range(n):
                                print(f"Training, use {self.comet_names[i]}")
                                batch = batch.union(self.comet_wg_list[i].compute_comet_rm(batch))

                        if self.use_embed:
                            batch = batch.union(self.embed_wg.compute_embed_rm(batch))
                        ...
                        if self.config.reward_model.launch_reward_fn_async:
                            ...
                            reward_tensor, reward_extra_infos_dict, ret_batch = compute_reward(batch, self.reward_fn)                        
                            if ret_batch:
                                batch = ret_batch
                                assert len(reward_tensor) == len(ret_batch)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        li = self.config.workflow.get("remove_gradient_depth_list", [])
                        if len(li) > 0:
                            depth = batch.non_tensor_batch.get("depth", None)
                            mask = ~np.isin(depth, li)
                            print(f"We will use {li} to remove some samples")
                            batch = batch[mask]
                            print(f"[INFO] We will use {len(batch)} sample to update policy")
