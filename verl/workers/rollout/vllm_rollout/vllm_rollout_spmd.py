...
#         self.config = config
#         client_args = {"transport": httpx.AsyncHTTPTransport(proxy=None)} if self.config.workflow.get("ignore_proxy", False) else {}
#         self.inference_engine = AsyncOpenAI(
#             api_key=self.config.workflow.get("api_key", None),
#             base_url=self.config.workflow.get("base_url"),
#             http_client=httpx.AsyncClient(**client_args),
#         )

#         self.model = self.config.workflow.get("model", None)
#         if self.model is None:
#             self.model = self.inference_engine.models.list().data[0].id
#     @torch.no_grad()
#     def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
#         idx = prompts.batch["input_ids"]  # (bs, prompt_length)
#         # left-padded attention_mask
...
#         # users can customize different sampling_params at different run
#         with self.update_sampling_params(**kwargs):
#             for x in vllm_inputs:
#                 prompt_id = x["prompt_token_ids"]

#             outputs = self.inference_engine.completions.create(
#                 model=self.config.workflow.get("model")
#                 prompts=vllm_inputs,  # because we have already convert it to prompt token id
#                 sampling_params=self.sampling_params,
#                 lora_request=lora_requests,
#                 use_tqdm=False,
